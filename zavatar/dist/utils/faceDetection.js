...
exports.detectFaceAndEstimateParams = detectFaceAndEstimateParams;
 ...
 */
async function detectFaceAndEstimateParams(imageBuffer, fallbackStyle) {
    try {
        ...
        const centerCrop = await (0, sharp_1.default)(imageBuffer)
...
function estimateSkinTone(r, g, b) {
    const avg = (r + g + b) / 3;
    if (avg > 220)
        return '#FDDFDF'; // Fair
    if (avg > 185)
        return '#F5CBA7'; // Light
    if (avg > 150)
        return '#E0AC69'; // Medium / Olive
    if (avg > 115)
        return '#C68642'; // Tan / Amber
    if (avg > 75)
        return '#8D5524'; // Deep Bronze
    return '#3B2219'; // Rich Dark
}
function getDefaultParams(style) {
    return {
